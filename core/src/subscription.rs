    ...
    H: std::hash::Hasher,
{
    ...
    pub fn from_recipe(recipe: impl Recipe<H, Input, Output = O> + 'static) -> Self {
        Self {
            recipes: vec![Box::new(recipe)],
        }
    }

    pub fn batch(subscription: impl Iterator<Item = Subscription<H, I, O>>) -> Self {
        Self {
    ...
    pub fn map<A>(mut self, f: impl Fn(O) -> A + Send + Sync + 'static) -> Subscription<H, I, O>
    where
        H: 'static,
        I: 'static,
        O: 'static,
...
pub trait Recipe<Hasher: std::hash::Hasher, Input> {
    ...
    fn stream(self: Box<Self>, input: Input) -> futures::stream::BoxStream<'static, Self::Output>;
}
    ...
    H: std::hash::Hasher,
{
    ...
    fn stream(self: Box<Self>, input: I) -> futures::stream::BoxStream<'static, Self::Output> {
